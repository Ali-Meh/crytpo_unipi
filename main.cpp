...
int deriveSharedKey(EVP_PKEY *dh_key, EVP_PKEY *peer_pubkey, unsigned char *sym_key, size_t *skey_len)
{
    *skey_len = 32;
    ...
    memcpy(sym_key, md_value, *skey_len); // Use the first 32 bytes as the key
...
public:
    ...
    void receive_key(EVP_PKEY *server_key)
    {
        ...
        deriveSharedKey(pub_key, server_key, (unsigned char *)&sym_key, &secret_length);
...
public:
    ...
    void receive_params(EVP_PKEY *client_key, DH *client_params)
    {
        ...
        size_t secret_length = 0;
        unsigned char shared_secret[32] = {0};
        deriveSharedKey(pub_key, client_key, (unsigned char *)&shared_secret, &secret_length);
